...
**Requires `ffmpeg` and `ffprobe`.** Check first — do not assume they are present:

```bash
command -v ffmpeg ffprobe || echo "MISSING"
```

If missing, install for the current platform (`apt-get install -y ffmpeg`,
`brew install ffmpeg`, or `winget install Gyan.FFmpeg`) before continuing. If
installation is not possible, say so and stop — the frame extraction in Step 2
